...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='deals' AND column_name='loan_amount') THEN
    ALTER TABLE public.deals ADD COLUMN loan_amount numeric;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='deals' AND column_name='revenue_annual') THEN
    ALTER TABLE public.deals ADD COLUMN revenue_annual numeric;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='deals' AND column_name='notes_internal') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='deals' AND column_name='email') THEN
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='deals' AND column_name='industry') THEN
    ALTER TABLE public.deals ADD COLUMN industry text;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='deals' AND column_name='state') THEN
    ALTER TABLE public.deals ADD COLUMN state text;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='deals' AND column_name='fico_score') THEN
    ALTER TABLE public.deals ADD COLUMN fico_score integer;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='deals' AND column_name='use_of_funds') THEN
    ALTER TABLE public.deals ADD COLUMN use_of_funds text;
  END IF;
  IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name='deals' AND column_name='cognito_raw') THEN
...
-- Ensure created_at exists and is not null
...
CREATE INDEX IF NOT EXISTS deals_cognito_entry_id_idx ON public.deals (cognito_entry_id);

-- Ownership model: never store NULL user_id; create system user reference row if needed
DO $$
BEGIN
...
BEGIN
  ...
  IF (u.email IN ('chris@gokapital.com','deals@gokapital.com','info@gokapital.com')) THEN
    RETURN TRUE;
  END IF;
...
-- INSERT: authenticated can insert (handled via functions), ensure user_id set
DROP POLICY IF EXISTS deals_insert ON public.deals;
CREATE POLICY deals_insert ON public.deals
FOR INSERT TO authenticated
WITH CHECK (user_id = auth.uid());

-- UPDATE (collaborative fields for all authenticated users)
DROP POLICY IF EXISTS deals_update_collab ON public.deals;
CREATE POLICY deals_update_collab ON public.deals
FOR UPDATE TO authenticated
USING (true)
WITH CHECK (
  -- Allow updating collaborative fields
  TRUE
);
